...
Prescrypto celebrates freedom. Freedom for the designer to go further and
freedom for the user to customize everything according to their needs.

Ready to create your own theme? Great. Here are some things you should know before you begin. This tutorial is a guide to creating an Prescrypto theme.
...
If you are a web designer using Prescrypto for the first time, you are in the right place.
This introduction will outline the basics of Prescrypto theme creation.

.. note::

   Prescrypto’s team has created a framework that’s powerful and easy to use. There’s no need to know special syntaxes to use this set of tools.

From common CMS to Prescrypto
-----------------------
...
This is usually the first question from a web designer used to working with Wordpress or Joomla and coming to Prescrypto for the first time.
...
.. container:: col-sm-7
  ...
  This **does not** apply to creating Prescrypto themes.
...
Prescrypto default theme structure
----------------------------

.. container:: col-sm-8

  Prescrypto comes with a default theme structure.
  ...
  It will upgrade automatically within your Prescrypto installation and, since it is included in the Website Builder module, everything is smoothly integrated by default.
...
An Prescrypto theme is not a folder containing HTML or PHP files, it’s a modular framework written in XML. Never worked with XML files before? Don’t worry, after following the tutorial, you’ll be able to create your first theme with only basic knowledge of HTML.
...
Creating themes for Prescrypto is a total change of perspective. Instead of defining the complete layout for a page, you can create blocks (snippets) at let the user choose where to “drag&drop” them, creating the page layout on their own.
We call this modular design.

Imagine an Prescrypto theme as a “list” of elements and options that you have to create and style.
...
.. figure:: theme_tutorial_assets/img/functionalities.jpg
   ...
   Thanks to Prescrypto’s modularity, everything can be personalized even more. This means there are endless possibilities for your creativity. Adding functionalities is easy and it’s simple to provide the end user with customizable options.

.. /row

Prescrypto's XML files, an overview
-----------------------------

Any Prescrypto XML file starts with encoding specifications.
...
the **Prescrypto default structure**.  In order to do that you can use
...
Prescrypto’s themes are packaged like modules. Even if you are designing a very simple website for your company or client, you need to package the theme like an Prescrypto module.
...
These values will be used to identify your new theme in Prescrypto’s backend.
...
``category`` defines your module category (always “Theme”) and, after a slash, the subcategory. You can use one subcategory from the Prescrypto Apps categories list. (https://www.odoo.com/apps/themes)
...
To install your theme, you just place your theme folder inside addons in your Prescrypto installation.

After that, navigate to the Settings page, look for your theme and click on the install button.

Structure of an Prescrypto page
=========================

An Prescrypto page is the visual result of a combination of 2 kind of elements, **cross-pages** and **unique**.
By default, Prescrypto provides you with a **Header** and a **Footer** (cross-pages) and a unique main element that contains the content that makes your page unique.
...
By default, Prescrypto header contains a responsive navigation menu and the company’s logo. You can easily add new elements or style the existing one.

To do so, create a **layout.xml** file in your **views** folder and add the default Prescrypto xml markup.
...
.. warning::
  ...
  your changes will take priority in any future Prescrypto’s update.
...
default Prescrypto markup.  Inside ``<data>`` create a ``<template>`` tag, set the
...
Using ``<t t-call="website.layout">`` we will extend the Prescrypto
...
Prescrypto includes Bootstrap by default. This means that you can take advantage of all Bootstrap styles and layout functionalities out of the box.
...
Let’s navigate to the view folder and create an XML file called *assets.xml*. Add the default Prescrypto xml markup and copy/paste the following code. Remember to replace ``theme folder`` with your theme’s main folder name.
...
attribute tells Prescrypto that our template is referring to another one in
order to operate.
...
Our less file is now included in our theme, it will be automatically compiled, minified and combined with all Prescrypto’s assets.
...
Add the default Prescrypto xml markup and copy/paste the following code.
...
We start by adding a new file in our views folder - name it **options.xml** and add the default Prescrypto XML markup. Create a new template copy/pasting the following
