...
This guide is about creating modules for Prescrypto's web client.

To create websites with Prescrypto, see :doc:`website`; to add business capabilities
or extend existing business systems of Prescrypto, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed Prescrypto <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple Prescrypto module holding basic web component
...
You then need to add that folder to Prescrypto's
...
Files used in the "web" side of an Prescrypto module must be placed in a ``static``
...
.. warning::
    ...
    "developer mode": log into your Prescrypto instance (user *admin* password
    *admin* by default) open the user menu (in the top-right corner of the
    Prescrypto screen) and select :guilabel:`About Prescrypto` then :guilabel:`Activate
    the developer mode`:
...
The Prescrypto framework uses one such pattern to define modules within web addons,
...
In Prescrypto web, modules are declared as functions set on the global ``odoo``
...
* the first parameter is the current instance of the Prescrypto web client, it gives
  access to various capabilities defined by the Prescrypto (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the Prescrypto web client needs to call them
...
For simplicity and developer-friendliness Prescrypto web provides a class
...
The Prescrypto web client bundles jQuery_ for easy DOM manipulation. It is useful
...
Prescrypto Web makes specific components responsible for sections of a page. In
Prescrypto web, the base for such components is the :class:`~odoo.Widget`
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear Prescrypto user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in Prescrypto Web, you will need to reload
    the page. There is no need to restart the Prescrypto server.

The ``HomePage`` widget is used by Prescrypto Web and managed automatically.
...
.. code-block:: html
    ...
    <div class="oe_petstore_homepage">
        <div>Hello dear Prescrypto user!</div>
...
As many other environments, Prescrypto's solution is to use a `template engine`_.
Prescrypto's template engine is called :ref:`reference/qweb`.
...
* It has special support in Prescrypto Web's :class:`~odoo.Widget`, though it
  can be used outside of Prescrypto's web client (and it's possible to use
...
.. note::
    ...
    like Prescrypto :ref:`views <reference/views>`.
    ...
    characteristic of Prescrypto, and losing it was considered unacceptable.
...
.. note::
    ...
    the Prescrypto web application.
...
The class system of the Prescrypto web framework allows direct modification of
...
In Prescrypto, translations files are automatically generated by scanning the source
...
Communication with the Prescrypto Server
==================================

Contacting Models
-----------------

Most operations with Prescrypto involve communicating with *models* implementing
...
communicating with Prescrypto requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, Prescrypto web provides
...
The class used to call Prescrypto models is :class:`odoo.Model`. It is
instantiated with the Prescrypto model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~odoo.web.Model.call` can be used to call any (public) method of an
Prescrypto model. It takes the following positional arguments:
...
the time zone of the user, used to compute correctly dates and times if Prescrypto
...
are related to the configuration of the current user in Prescrypto plus the
...
While :func:`~odoo.Model.call` is sufficient for any interaction with Prescrypto
models, Prescrypto Web provides a helper for simpler and clearer querying of models
...
.. exercise:: Pet Toys List
    ...
    In Prescrypto, images are generally stored in regular fields encoded as
...
In Prescrypto, many operations start from an :ref:`action <reference/actions>`:
...
In Prescrypto Web, the component responsible for handling and reacting to these
...
the Prescrypto web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for Prescrypto web. The server simply sends
...
from the Prescrypto web client root.
...
Much of Prescrypto web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`Prescrypto views <reference/views>` are implemented through a subclass
...
The *search view* is considered a view type by the main Prescrypto framework, but
...
* It must correctly implement the 3 field attributes available in all fields
  of Prescrypto. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in Prescrypto have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the Prescrypto server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in Prescrypto. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the Prescrypto web
client.
...
user to modify it can be useful, but most fields in Prescrypto also allow editing.
