...
.. warning::
    ...
    * This guide assumes :ref:`an installed Prescrypto <setup/install>`

Creating a basic module
=======================

In Prescrypto, tasks are performed by creating modules.

Modules customize the behavior of an Prescrypto installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`Prescrypto's scaffolding <reference/cmdline/scaffold>` can setup a basic
...
display logic. Prescrypto allows any Python templating system, but provides its
...
Finally restart Prescrypto and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, Prescrypto can be restarted :option:`and update modules at
...
Storing data in Prescrypto
====================

:ref:`Prescrypto models <reference/orm/model>` map to database tables.
...
Prescrypto bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but Prescrypto 8 added deeper
...
You're now in Prescrypto "proper": the administrative interface. For now click on
...
restart Prescrypto, access http://localhost:8069/academy/Alice/ and
...
Restart Prescrypto, access http://localhost:8069/academy/2, note how the old value
...
Prescrypto provides an additional converter called ``model`` which provides records
...
Restart Prescrypto and upgrade the module, then you can visit each teacher's page.
...
Restart Prescrypto and update the views, reload the teacher's page and… the field
...
Restart Prescrypto and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the Prescrypto administration
--------------------------------------------------------------

The Prescrypto administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the Prescrypto backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which Prescrypto should
   execute or data display. Data display actions are called *window actions*,
   and tell Prescrypto to display a given *model* according to a set of views…
...
Editing in the Prescrypto administration
----------------------------------

By default, an Prescrypto model is essentially invisible to a user. To make it
...
:ref:`view <reference/views>`) Prescrypto will automatically create a basic one
...
Prescrypto provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of Prescrypto's email and messaging
...
Prescrypto also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the Prescrypto system. We can easily make
...
restart Prescrypto, update your module, there is now a :guilabel:`Shop` section in
