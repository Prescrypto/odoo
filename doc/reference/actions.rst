...
Allow opening a URL (website/web page) via an Prescrypto action. Can be customized
via two fields:
...
will replace the current content section by the Prescrypto home page.
...
``model_id``
    Prescrypto model linked to the action, made available in
