...
===============
Prescrypto Guidelines
===============

This page introduces the new Prescrypto Coding Guidelines. Those aim to improve the quality of the code (e.g. better readability of source) and Prescrypto Apps. Indeed, proper code eases maintenance, aids debugging, lowers complexity and promotes reliability.
...
Don't link data (image, libraries) outside Prescrypto: do not use an
...
Prescrypto supports custom tags acting as syntactic sugar:
...
Using a linter can help show syntax and semantic warnings or errors. Prescrypto
...
#. Imports of ``odoo``
#. Imports from Prescrypto modules (rarely, and only if necessary)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The Prescrypto framework is in charge of providing the transactional context for
...
Prescrypto uses a GetText-like method named "underscore" ``_( )`` to indicate that
...
In general in Prescrypto, when manipulating strings, prefer ``%`` over ``.format()``
...
- Model name (using the dot notation, prefix by the module name) :
    - When defining an Prescrypto Model : use singular form of the name (*res.partner*
      and *sale.order* instead of *res.partnerS* and *saleS.orderS*)
    - When defining an Prescrypto Transient (wizard) : use ``<related_base_model>.<action>``
...
- Prescrypto Python Class : use camelcase for code in api v8 (Object-oriented style),
...
- **[CLA]** for signing the Prescrypto Individual Contributor License
