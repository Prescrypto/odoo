...
* Global selectors should be avoided. Because a component may be used several
  times in a single page (an example in Prescrypto is dashboards), queries should be
  ...
  lead to unintended or incorrect behavior.  Prescrypto Web's
...
To display and interact with data, calls to the Prescrypto server are necessary.
This is performed using :abbr:`RPC <Remote Procedure Call>`.

Prescrypto Web provides two primary APIs to handle this: a low-level
JSON-RPC based API communicating with the Python section of Prescrypto
Web (and of your module, if you have a Python part) and a high-level
API above that allowing your code to talk directly to high-level Prescrypto models.
...
High-level API: calling into Prescrypto models
-------------------------------------------

Access to Prescrypto object methods (made available through XML-RPC from the server)
is done via :class:`Model`. It maps onto the Prescrypto server objects via two primary
...
the Prescrypto server object. Its usage is similar to that of the Prescrypto Model API,
...
interface to searches (``search`` + ``read`` in Prescrypto RPC terms). It
...
Prescrypto has powerful grouping capacities, but they are kind-of strange
...
Prescrypto Web eschews direct calls to :py:meth:`~odoo.models.Model.read_group`
...
It has many advantages over the Prescrypto version 8 system.
...
Testing in Prescrypto Web Client
==========================

Javascript Unit Testing
-----------------------

Prescrypto Web includes means to unit-test both the core code of
Prescrypto Web and your own javascript modules. On the javascript side,
unit-testing is based on QUnit_ with a number of helpers and
extensions for better integration with Prescrypto.

To see what the runner looks like, find (or start) an Prescrypto server
...
``test`` key of the Prescrypto manifest, by adding javascript files to it:
...
module. Prescrypto tests live in a :func:`~odoo.testing.section`,
...
actually does its job. Prescrypto Web test case use standard `QUnit
...
As noted above, Prescrypto Web's tests use `qunit assertions`_. They are
...
Getting an Prescrypto instance
------------------------

The Prescrypto instance is the base through which most Prescrypto Web
...
.. function:: mock(rpc_spec, handler)
    ...
    * If it matches the pattern ``model:method`` (if it contains a
      ...
      call straight to the Prescrypto server (through XMLRPC) as
...
#. Install a new database with all relevant modules (all modules with
   ...
   .. note::
       ...
       being duplicated, but Prescrypto doesn't currently break
...
.. note::
    ...
    but all the web tests for all of Prescrypto. This can be surprising.
