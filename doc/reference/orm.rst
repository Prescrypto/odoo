...
.. versionadded:: 8.0
    ...
    This page documents the New API added in Prescrypto 8.0 which should be the
...
Prescrypto maintains a cache for the fields of the records, so that not every field
...
To avoid reading one field on one record at a time, Prescrypto *prefetches* records
...
:meth:`~odoo.models.Model.browse`
    ...
    record ids are obtained from outside Prescrypto (e.g. round-trip through
...
Prescrypto is currently transitioning from an older (less regular) API, it can be
...
Prescrypto provides three different mechanisms to extend models in a modular way:
...
:attr:`~odoo.models.Model._name` attributes together, Prescrypto creates a new
...
* double check that all fields and methods have different names, there is no
  warning in case of collision (because Python handles it before Prescrypto sees
  anything)
