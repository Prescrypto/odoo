...
This document describes the API used to upgrade an Prescrypto database to a
higher version.
...
* your email address
* the target version (the Prescrypto version you want to upgrade to)
* the purpose of your request (test or production)
* the database dump name (required but purely informative)
* optionally the server timezone (for Prescrypto source version < 6.1)
...
.. py:function:: https://upgrade.odoo.com/database/v1/create
    ...
    :param str target: (required) the Prescrypto version you want to upgrade to. Valid choices: 6.0, 6.1, 7.0, 8.0
    ...
    :param str timezone: (optional) the timezone used by your server. Only for Prescrypto source version < 6.1
...
``target``
    the target Prescrypto version you supplied when creating the request
``aim``
...
``issue_stage``
    the stage of the issue we have create on Prescrypto main server
``issue``
    the id of the issue we have create on Prescrypto main server
``status_url``
...
``database_version``
    the guessed Prescrypto version of your uploaded (not upgraded) database
``postgresql``
