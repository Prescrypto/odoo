...
In Prescrypto, a workflow is a technical artefact to manage a set of "things to
...
- Activities define work that should be done within the Prescrypto server, such
...
All in all, Prescrypto's workflow system provides:
...
A company using Prescrypto may want to add discount support to orders, where sales
...
Prescrypto knows where to start the workflow traversal after it is instanciated.
...
for a record, Prescrypto simply processes all of them, and evaluate all their
...
It is important for Prescrypto to know when a workflow instance is completed. A
...
giving a signal name in the attribute ``signal_send``. Prescrypto processes those
...
After an activity has been processed, Prescrypto evaluates its transition to reach
the next activity in the flow.

However if an activity has more than one transition, Prescrypto must decide which
...
``XOR`` (default)
    By default, Prescrypto will use the first transition (in ``sequence`` order)
...
``OR``
    ...
    become valid later.
``AND``
    In ``AND`` mode, Prescrypto will wait until *all* transitions are satisfied, and
...
signal or trigger is defined), the condition is evaluated by Prescrypto, and if
...
defined (in addition to the Prescrypto ``safe_eval`` environment):
