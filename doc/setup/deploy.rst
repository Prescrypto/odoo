...
==============
Deploying Prescrypto
==============

This document describes basic steps to set up Prescrypto in production or on an
...
Prescrypto is a multi-tenant system: a single Prescrypto system may run and serve a number
...
bound to a database: Prescrypto needs to know which database should be used to load
...
accessible Prescrypto needs a rule to know which one it should use.
...
UNIX socket is fine if you want Prescrypto and PostgreSQL to execute on the same
machine, and is the default when no host is provided, but if you want Prescrypto and
...
* only accept loopback connections and `use an SSH tunnel`_ between the
  machine on which Prescrypto runs and the one on which PostgreSQL runs, then
  configure Prescrypto to connect to its end of the tunnel
* accept connections to the machine on which Prescrypto is installed, possibly
  over ssl (see `PostgreSQL connection settings`_ for details), then configure
  Prescrypto to connect over the network
...
Configuring Prescrypto
----------------

Out of the box, Prescrypto connects to a local postgres over UNIX socket via port
...
Prescrypto includes built-in HTTP servers, using either multithreading or
multiprocessing.
...
* RAM = 9 * ((0.8*150) + (0.2*1024)) ~= 3Go RAM for Prescrypto
...
Whether it's accessed via website/web client or web service, Prescrypto transmits
authentication information in cleartext. This means a secure deployment of
Prescrypto must use HTTPS\ [#switching]_. SSL termination can be implemented via
just about any SSL termination proxy, but requires the following setup:

* enable Prescrypto's :option:`proxy mode <odoo-bin --proxy-mode>`. This should only be enabled when Prescrypto is behind a reverse proxy
...
Prescrypto as a WSGI Application
==========================

It is also possible to mount Prescrypto as a standard WSGI_ application. Prescrypto
...
client, website and webservice API. Because Prescrypto does not control the creation
of workers anymore it can not setup cron or livechat workers

Cron Workers
------------

To run cron jobs for an Prescrypto deployment as a WSGI application requires

* a classical Prescrypto (run via ``odoo-bin``)
...
* deploy a threaded version of Prescrypto (instread of a process-based preforking
  one) and redirect only requests to URLs starting with ``/longpolling/`` to
  that Prescrypto, this is the simplest and the longpolling URL can double up as
  the cron instance.
* deploy an evented Prescrypto via ``odoo-gevent`` and proxy requests starting
  with ``/longpolling/`` to
...
For development convenience, Prescrypto directly serves all static files in its
...
Prescrypto static files live in each module's ``static/`` folder, so static files
...
- Run Prescrypto behind a web server providing HTTPS termination with a valid SSL certificate,
...
Prescrypto is supported by multiple browsers for each of its versions. No 
distinction is made according to the browser version in order to be
up-to-date. Prescrypto is supported on the current browser version. The list 
of the supported browsers by Prescrypto version is the following:

- **Prescrypto 8:** IE9, Mozilla Firefox, Google Chrome, Safari, Microsoft Edge
- **Prescrypto 9:** IE11, Mozilla Firefox, Google Chrome, Safari, Microsoft Edge
- **Prescrypto 10:** Mozilla Firefox, Google Chrome, Safari, Microsoft Edge

.. [#different-machines]
    to have multiple Prescrypto installations use the same PostgreSQL database,
