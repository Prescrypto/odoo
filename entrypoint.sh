...
case "$1" in
    -- | odoo)
        ...
        if [[ "$1" == "scaffold" ]] ; then
            ...
            exec /odoo/odoo-bin --database=${DB_ENV_DB_NAME} --db_user=${DB_ENV_POSTGRES_USER} --db_password=${DB_ENV_POSTGRES_PASSWORD} --db_host=${DB_PORT_5432_TCP_ADDR} --db_port=${DB_PORT_5432_TCP_PORT}
        fi
        ;;
    -*)
        wait-for-psql.py ${DB_ARGS[@]} --timeout=30
